...
export const PreviewTerminal = ({ output }: PreviewTerminalProps) => {
  ...
  useEffect(() => {
    ...
    // "output" does need to be a dependency since it is not intended to update anything, just used on mount
  }, []);
  ...
  return (
    <div
      ref={containerRef}
      className="flex-1 min-h-0 p-3 [&_.xterm]:h-full!
  [&_.xterm-viewport]:h-full! [&_.xterm-screen]:h-full! bg-sidebar"
    />
  );
};
